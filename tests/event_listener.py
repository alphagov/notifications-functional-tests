...
class Event:
    ...
    def __repr__(self):
        ...
        return (
            f"Event({self.first_timestamp.time().isoformat()}, {self.event_type}, {url=}, {self.data=}"
            # only print num tries if there was more than 1
            + (f", {self.attempts=}" if self.attempts != 1 else "")
            # only print last_timestamp if it != first_timestamp
            + (
                f", self.last_timestamp={self.last_timestamp.time().isoformat()}"
