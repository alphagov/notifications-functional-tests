...
def add_file_to_an_email_template(driver, template_name, file_name, service_id):
    show_templates_page = ShowTemplatesPage(driver)
    try:
        show_templates_page.click_template_by_link_text(template_name)
    except TimeoutException:
        ...
        dashboard_page.click_templates()
        show_templates_page.click_template_by_link_text(template_name)
    template_page = ViewEmailTemplatePage
    template_page.click_attach_files_button()
    add_a_file_page = AddFileToEmailTemplatePage(driver)

    assert add_a_file_page.visible_choose_file_button()
    add_a_file_page.upload_file()
    add_a_file_page.upload_file(os.path.join(os.getcwd(), file_name))
