...
8. On your local, `PASSWORD_STORE_DIR=$HOME/gds/notifications-credentials pass edit credentials/functional-tests/preview-functional`  and update the new environment file. This will auto commit and require manual PR process.
  * Set `REQUEST_BIN_API_TOKEN` in the environment when editing credentials
9. After merging the credentials PR, run `notifications-aws/scripts/upload-credentials/upload-credentials.sh` preview locally to push new environment file to SSM.
